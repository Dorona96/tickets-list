...
import ticketLogoDark from "../src/TicketListDark.png";
...
import sunLogo from "../src/sun.jpg";
import moonLogo from "../src/moon.png";
...
export class App extends React.PureComponent<{}, AppState> {
      ...
      <ul className="tickets" >
        {filteredTickets.map((ticket) => (
          <li key={ticket.id} className="ticket" >
            ...
            <button id="rename" onClick={()=>this.handleRename(ticket.id)}>
              Rename
            </button>
            ...
            <footer>
              ...
              <div id="labels">{ticket.labels+""}</div>
            </footer>
          </li>
        ))}
      </ul>
  ...
  handleRename =(id:string)=>{
    ...
    const newT = this.state.tickets.filter((ticket) => ticket.id !== id);
    const newC = this.state.tickets.filter((ticket) => ticket.id === id);
    const answer = String(
      window.prompt("Type the new title: ")
    );
    newT[this.state.changeTitle].title=answer;
    var margeT = newT.concat(newC);
      this.setState({
        tickets:margeT, changeTitle:this.state.changeTitle+1
      });
    }
  }
  ...
  render() {
    ...
    return (
      <main>
        <button className="mode" onClick={() => this.handleMode()}>
          {this.state.theme==="light"&&<img src={sunLogo}/>}
          {this.state.theme==="dark"&&<img src={moonLogo} />} 
        </button>
        <h1>
         {this.state.theme==="light"&&<img src={ticketLogo}/>} 
         {this.state.theme==="dark"&&<img src={ticketLogoDark} />} 
        </h1>
